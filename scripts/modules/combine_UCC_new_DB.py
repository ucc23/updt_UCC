...
def main(
    logging, new_DB_ID, df_UCC, df_new, json_pars, new_DB_fnames,
    db_matches, sep
):
    """
    """
    ...
    for i, new_cl in enumerate(new_DB_fnames):
        ...
        if db_matches[i] is None:  # The cluster is not present in the UCC
            new_db_dict = new_OC_not_in_UCC(
                new_DB_ID, new_db_dict, i, new_cl, row_n, new_names, ra_c,
                dec_c, plx_c, pmra_c, pmde_c)
            ...
            new_db_dict = OC_in_UCC(
                new_DB_ID, new_db_dict, i, new_cl, new_names, row)
            logging.info(
                f"{i} {','.join(new_cl)} is in the UCC: update DB indexes")
...
def new_OC_not_in_UCC(
    new_DB_ID, new_db_dict, i, new_cl, row_n, new_names, ra_c, dec_c, plx_c,
    pmra_c, pmde_c
):
    """
    """
