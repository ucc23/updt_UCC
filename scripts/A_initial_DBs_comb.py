...
def main():
    ...
    OCs_names, sep, dbs_folder, all_DBs_json, UCC_folder, new_OCs_fpath = \
        pars_dict['OCs_names'], pars_dict['sep'], pars_dict['dbs_folder'], \
        pars_dict['all_DBs_json'], pars_dict['UCC_folder'], \
    ...
    DB_data, DB_fnames, DB_names_orig = get_data_and_names(
        dbs_folder, dbs_used, sep)
    ...
    cols_order = [
        "DB", "DB_i", "ID", "RA_ICRS", "DE_ICRS", "GLON", "GLAT", "plx",
        "pmRA", "pmDE", "fnames", "UCC_ID", "quad", "dups_fnames",
        "dups_probs", "r_50", "N_fixed", "N_membs", "fixed_cent",
        "cent_flags", "C1", "C2", "C3", "N_50", "GLON_m", "GLAT_m",
        "RA_ICRS_m", "DE_ICRS_m", "plx_m", "pmRA_m", "pmDE_m", "Rv_m",
        "N_Rv", "dups_fnames_m", "dups_probs_m"]
    comb_dbs = comb_dbs[cols_order]

    # Save to file
    pd.DataFrame(comb_dbs).to_csv(
        UCC_folder + 'UCC_corrected' + '.csv', na_rep='nan', index=False,
