...
fetch("clusters.json.gz")
  .then(res => res.arrayBuffer()) // Fetch as binary data
  .then(buffer => {
    const decompressed = pako.ungzip(new Uint8Array(buffer), { to: 'string' });
    const data = JSON.parse(decompressed); // Parse the JSON data
  ...
  .catch(error => console.error('Error fetching or decompressing file:', error));
