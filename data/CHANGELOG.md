...
- Updated B file with new preferred coordinates logic and hierarchy
- Updated some OCs:
  - alessi58: "b_3.5"
  - bochum1: alessi58 on top, "t_3.5"
  - collinder132: N_clust=75
  - feigelson1: N_clust_max=75
  - fsr0569: "l_130.1,t_11.4"
  - lp44: "l_203.15" (NGC 2264 to the left)
  - ngc1976: N_clust_max=75
  - ngc7086a: similar (ra, dec) as ngc7086, but very different pms, use Loktin values
  - ngc7086b: similar (ra, dec) as ngc7086, but very different pms, use Loktin values
  - ocsn244: N_clust_max=75
  - ocsn246: N_clust_max=100
  - trumpler14: tr15 in frame, "t_-0.5"
  - trumpler15: tr14 in frame, "b_-0.5"
  - trumpler37: N_clust=100
  - ubc1309: dup of Bochum1, alessi58 on top, "t_3.5"
  - ubc491: VDBH92 to the left in plx; "plxl_0.22"
  - ascc18: "plxr_2.55", bad PMs from Donor, use Hunt values
  - ascc20: "plxl_2.55", bad PMs from Donor, use Hunt values
  - hsc517: N_box=1.5, N_clust_max=75
  - ngc2264: "r_203.15"
  - rsg7: rsg8 in frame, "plxl_2.2", bad PMs from Roser, use Cantat values
  - rsg8: rsg7 in frame, "plxr_2.2", bad PMs from Roser, use Cantat values
  - ubc602: N_clust=100, Bad pm coords in CANTAT2020, correct ones are those in HUNT2023



## 251222-260102

- Major changes to the site's look and feel. No changes to OCs database
...
Remove KHARCHENKO2012 (HEARSAC compilation) and replace by the actual articles
(and a few more) from Kharchenko et al. spanning 2003 to 2023.

Steps:

1. Remove KHARCHENKO2012, UCC cat is reduced from 16746 to 16550 entries
2. Add Kharchenko 2003
3. Add Kharchenko 2005
4. Add Kharchenko 2005_1
5. Added Kharchenko 2007
6. Added Kharchenko 2012
7. Added Kharchenko 2013
8. Added Schmeja 2014
9. Added Scholz 2015
10. Added Kharchenko 2016
11. Added Yen et al 2018
12. Added Just et al 2023
...
- Renamed in Loktin (2017): MWSC0 192 --> MWSC 192
- Cavallo et al 2024: added 'VDBH' to 'BH' entries
...
- Added UBC 32 as primary denomination of UPK 19 in SIM2019 (as explained in CASTRO2020)
Four CASTRO2018 & CASTRO2019 UBC names were already identified as primary denominations
to FoF entries in the new LIUPANG2019, so no change was required.
- Modified LIUPANG2019 back to the previous version


## 25111410

- Updated the LIUPANG2019 database with the complete one that includes their 
entire Table 1 (almost 1500 new FoF entries)


## 25111210

- Added back five entries in DIAS2002 with class 'e' (embedded) that were originally
  removed
- Removed "DB XX" denomination from "Dutra-Bica" entries
- Modified a few DBs in CASTRO2018 and LIUPANG2019 to add primary duplicated names
- Renamed 74 entries after modifying the B script to order fnames according to year


## 25110710

- HE2022_1 contains 14 entries named 'H21OC_*' that are 'hxhwl*' entries from
He et al 2021. Renamed them as follows:
...
- Added (X_GC, Y_GC, Z_GC, R_GC) columns with Galactocentric coordinates


## 25102509-25102915

- Added the FROEBRICH2007 database
- Re-arranged some main fnames
- Removed 'fnames' column from C and kept only the first fname
