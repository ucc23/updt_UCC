...
- The [updt_ucc](https://github.com/ucc23/updt_UCC) repository, this one, contains the scripts and data files
  ...
  main files that make up the UCC: the `UCC_cat_XXYYZZ.csv` catalogue, and the
  `UCC_members.parquet` file with all the identified members. Both files are stored
...
- The eight `QXX` repositories ([Q1P](https://github.com/ucc23/Q1P), etc.) contain several plots per OC
  in the `plots` folders: one with the four diagrams, one for Aladin,
  and plots for the HUNT23 and/or CANTAT20 members when available. The plots are loaded
...
`all_dbs.json` file with new DBs to add.
...
If the UCC is rebuilt we need to check carefully that the main `fnames` did not change
and if they did **all** the associated file names should be modified accordingly:

1. The .parquet file with its identified members
2. The .md file that generates its page in the ucc.ar site
3. The several plots that are associated with the entry in the `QXY/plots/` folders


### Input

- `zenodo/UCC_cat_B.csv`: Current version of this file
...
The `C_process_member_files.py` script updates the `UCC_cat_B.csv` file and the parquet
file with all the identified members. It also generates the required files for
...
```
-plots_used: y/n,
-process: y/n, indicates whether to process this entry
...
Columns estimated using members in C script
- "r_50","C1","C2","C3","N_50","GLON_m","GLAT_m","RA_ICRS_m","DE_ICRS_m","Plx_m","pmRA_m","pmDE_m","Rv_m","N_Rv","shared_members","shared_members_p","N_clust","N_clust_max"
