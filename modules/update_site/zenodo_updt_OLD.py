...
def count_membs_UCC(root_UCC_path: str, members_folder: str) -> int:
    """
    Count the total number of members in all the UCC clusters.
    """
    N_tot = 0
    for quad in ("1", "2", "3", "4"):
        for lat in ("P", "N"):
            ...
            for file in os.listdir(path):
                ...
                if "HUNT23" in file or "CANTAT20" in file:
                    continue
                # Use Parquet metadata to get row count
                parquet_file = ParquetFile(os.path.join(path, file))
                N_tot += parquet_file.count()
